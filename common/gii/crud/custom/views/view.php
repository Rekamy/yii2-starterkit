...
<div class="<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-view">
    ...
    <?= "            <?= Html::a(" . $generator->generateString('Save As New') . ", ['save-as-new', " . $generator->generateUrlParams() . "], ['class' => 'btn btn-sm btn-info']) ?>" ?>
    <?php endif;?>
                <?= "
                <?= Html::a(" . $generator->generateString('Update') . ", ['update', " . $generator->generateUrlParams() . "], ['class' => 'btn btn-sm btn-primary']) ?>
        ...
        <div class="box-body">
            ...
            <div class="nav-tabs-custom">
                ...
                <ul class="nav nav-tabs">
                    <li class="active"><a href="#<?= StringHelper::basename($generator->modelClass) ?>" data-toggle="tab"><?= "<?= " ?><?= $generator->generateString(Inflector::camel2words(StringHelper::basename($generator->modelClass))) ?><?= " ?>" ?></a></li>
<?php foreach ($relations as $name => $rel): ?>
<?php if ($rel[2] && isset($rel[3]) && !in_array($name, $generator->skippedRelations)): ?>
                    <li><a href="#<?= $rel[1] ?>" data-toggle="tab"><?= "<?= " ?><?= $generator->generateString(Inflector::camel2words($rel[1])) ?> <?= "?>" ?></a></li>
...
<?php endforeach; ?>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="<?= StringHelper::basename($generator->modelClass) ?>">
                        <?= "<?php \n" ?>
                        $gridColumn = [
<?php
